...
class FindValidClass:
    ...
    def check_is_python(self, path):
        if(py_compile.compile(path) == None):
            return False
        return True
    ...
    def check_upload_valid(self,user_id, case_id, upload):
        ...
        :return: boolean
        '''
        filepath = self.python_source + user_id + "/" + case_id + "/" + upload
        ...
        with open(filepath, 'r') as f:
            try:
                ...
                if self.check_if_else(text) and self.check_specific(text) \
                        and self.check_too_short(text):
                    if(self.check_is_python(filepath)):
