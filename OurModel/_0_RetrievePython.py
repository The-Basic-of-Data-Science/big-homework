...
class RetrieveClass:
    ...
    def retrieve_upload(self, user_id, case_id, upload_id, url):
        ...
        # 如果已经有了就别重复写了
        if(os.path.exists(self.output + "/" + user_id + "/" + case_id)):
            filenames = os.listdir(self.output + "/" + user_id + "/" + case_id)
            for filename in filenames:
                if(str(upload_id) == filename.split("_")[0]):
                    return
        for x in range(1, 10):
            try:
    ...
    def retrieve_uploads(self):
        ...
        with open(self.source, 'r', encoding='utf-8') as f:
            ...
            for user_id in data.keys():
                user_number += 1
                print("Thread:" + str(self.name) + ",UserNumber:" + str(user_number))
